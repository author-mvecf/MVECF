...
import numpy as np
import pandas as pd
from lib.models.lightGCN.evaluation.python_evaluation import (
    map_at_k,
    ndcg_at_k,
    precision_at_k,
    recall_at_k,
)
from lib.models.lightGCN.utils.python_utils import get_top_k_scored_items
...
class LightGCN(object):
    ...
    def __init__(self, hparams, data: ImplicitCF, seed=None, result_dir=None, early_stop=True,
        ...
        self.data = data
        self.data_laod_func = self.data.data_loader
    ...
    def fit(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for idx in range(n_batch):
                users, pos_items, neg_items = self.data_laod_func(self.batch_size)
    ...
    def print_and_save_results(self, epoch, train_time, loss, mf_loss, emb_loss):
        ...
        self.list_stop_cri.append(loss)

        result_dict = {
            "pu": self.ua_embeddings.eval(session=self.sess),
            "qi": self.ia_embeddings.eval(session=self.sess),
            "lr": self.lr,
            "reg_param": self.decay,
            "batch_size": self.batch_size,
            "n_factors": self.emb_dim,
            "n_layers": self.n_layers,
            "n_users": self.data.n_users,
            "n_items": self.data.n_items,
            "list_stop_cri": self.list_stop_cri,
            "start_epoch": epoch + 1,
            "train_loss": loss,
            "gamma": self.gamma,
            "reg_param_mv": self.reg_param_mv,
            "alpha": self.data.alpha
        }
        if self.result_dir is not None:
            if not os.path.exists(self.result_dir):
                os.makedirs(self.result_dir)
            save_pickle(result_dict, os.path.join(self.result_dir, f"epoch_{epoch}.pkl"))
    ...
    def score(self, user_ids, remove_seen=True):
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            rate_batch = self.sess.run(
                self.batch_ratings, {self.users: user_batch, self.items: item_batch}
            )
